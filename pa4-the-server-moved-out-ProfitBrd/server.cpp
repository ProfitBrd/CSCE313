...
int main (int argc, char* argv[]) {
	...
	while (true) {
		int socket = control_channel->accept_conn();
		TCPRequestChannel* control_channel = new TCPRequestChannel(socket); //wtf
		thread temp(handle_process_loop, control_channel);
		temp.detach();
	}
