...
int main(int argc, char **argv)
{
	...
	/* TODO This Section - 1 */
	// Associate the signal SIGRTMIN with the sa using the sigaction function // 1 line!!!
	...
	while (recipeSteps->Count() != completeCount){
		vector<Step*> recipeStepsReady = recipeSteps->GetReadySteps();
		// if(!recipeStepsReady.empty()){
			for (auto item : recipeStepsReady){
				item->running = true;
				makeTimer(item, item->duration);
			}
